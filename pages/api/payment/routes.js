...
export default async function handler(req, res) {
  ...
  if (req.method == "GET") {
    try {
      // Query the database
      const q =
        "SELECT mp.*, DATE_FORMAT(mp.date, '%d-%m-%Y') as date, mm.fname, mm.mname, mm.lname, mm.nickname, mm.address, mm.mobile_no, mm.alt_mobile_no, mm.email, mm.aadhar_card, mm.bank_ac, mm.ifsc, mm.add_by, mm.date as member_date, mm.update_by as member_updated_by, mm.update_date as member_updated_date, cat.name AS category_name FROM cf_main_payment AS mp JOIN cf_member_master mm ON mp.m_id = mm.id JOIN cf_category cat ON mp.c_id = cat.id order by mp.id desc";
      //     `
      //   SELECT
      //     mp.*,
      //     DATE_FORMAT(mp.date, '%d-%m-%Y') as date,
      //     mm.fname,
      //     mm.lname,
      //     mm.nickname,
      //     mm.mname,
      ...
      //     mm.ifsc,
      //     mm.add_by,
      //     mm.date as member_date,
      //     mm.update_by as member_updated_by,
      //     mm.update_date as member_updated_date,
      //     cat.name AS category_name
      //   FROM
      //     cf_main_payment AS mp
      //   JOIN
      //     cf_member_master mm ON mp.m_id = mm.id
      //   JOIN
      //     cf_category cat ON mp.c_id = cat.id
      //   WHERE
      //     cat.name != 'lillah'
      //   ORDER BY
      //     mp.id DESC
